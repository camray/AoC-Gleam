...
pub fn day_1_part_2() -> Result(Nil, Nil) {
...
pub fn subtract(running: RunningResult, value: Int) -> RunningResult {
  let ending_location = case { running.location - value } % 100 {
    n if n < 0 -> n + 100
    n -> n
  }

  let zero_hits = case value >= running.location {
    True -> {
      case running.location {
        0 -> value / 100
        _ -> 1 + { value - running.location } / 100
      }
    }
    False -> 0
  }
  ...
  RunningResult(
    ...
    zero_hits: running.zero_hits + zero_hits,
  )
}
...
pub fn add(running: RunningResult, value: Int) -> RunningResult {
  ...
  let assert Ok(zero_hits) = int.floor_divide(running.location + value, 100)

  RunningResult(location: location, zero_hits: running.zero_hits + zero_hits)
}
